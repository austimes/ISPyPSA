...
Both the preceding demand/carbon sweep and intensity map disabled the optional CCS supply curve. They used a flat **A$89.93 per captured tonne** transport-and-storage charge. This port preserves that treatment. The compatibility argument `--ccs-supply-curve none` is accepted; a curve filename is rejected because the separate sink/injectivity feature is not included. This assumption does not establish physical disposal capacity.
...
Each solve record now includes the source Git commit, whether the worktree had uncommitted changes, and the SHA-256 of its generated YAML. Unavailable provenance is marked explicitly. This does not hash the large input package; preserve that package separately.
...
The original campaign matrix, workload manifest and Slurm submissions remain on `extension-campaign`. This workflow accepts their explicit demand/cap schedules but does not launch that campaign automatically. For exact historical reproduction, use its recorded source snapshot and input package rather than treating newer model fixes as unchanged history.
...
```text
uv run python analysis/benchmarks/build_trajectory_demand_dirs.py --source data/trace_data_final/isp_2026 --out-root outputs/inputs/traces --plan path/to/demand-plan.json
uv run python analysis/benchmarks/hold_supply_curves.py --curve analysis/gas_market/gas_supply_curve_central.csv --out outputs/inputs/gas_held.csv --years 2060
uv run python analysis/benchmarks/hold_supply_curves.py --curve analysis/bioenergy_market/biomass_supply_curve_central.csv --out outputs/inputs/biomass_held.csv --years 2060
```
