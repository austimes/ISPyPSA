...
trajectory, pressure setting). Every chain is solved by ``msm solve`` over the
milestones 2030/2040/2050/2060, and differs from its neighbours only in the
pressure flag this module writes: either ``--carbon-price N`` or
``--co2-cap-t-schedule YEAR:TONNES ...``.
...
* ``chains.tsv`` -- headerless ``run_id<TAB>traces<TAB>args``, one line per
  chain, where ``traces`` is the absolute path of that trajectory's
  ``<trajectory>.txt`` trace-directory token file. The line number is the Slurm
  array index, so the order is part of the contract: the uncapped chains first,
  then the price calibration chains, then the cap chains shallow to deep with
  the trajectories grouped inside each schedule.
...
* ``demand_plan.json`` -- a copy of the plan the manifest was built from.
"""
...
CHAINS_COLUMNS = ["row", "run_id", "trajectory", "traces", "chain", "stage", "args"]
...
def write_manifest(
    caps: pd.DataFrame, chains: pd.DataFrame, out_dir: Path, plan: Path
) -> None:
