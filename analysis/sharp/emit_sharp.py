"""Emit ShARP-shaped CSVs from one or more solved ISPyPSA archetype runs.
...
from .method_years import extract_method_year_row
...
def _find_archetype_runs(
    runs_dir: Path, run_id_prefix: str | None = None
) -> dict[str, list[Path]]:
...
def _emit_method_years_csv(rows: list[dict], out_dir: Path) -> Path:
        ...
        [
            {
                ...
                "diagnostic_bundled_cost_AUD_per_MWh": r[
                    "diagnostic_bundled_cost_per_unit"
                ],
                "diagnostic_fuel_cost_AUD_per_MWh": r["diagnostic_fuel_cost_per_unit"],
                "fuel_share_of_total_cost": (
                    r["diagnostic_fuel_cost_per_unit"]
                    / r["diagnostic_bundled_cost_per_unit"]
                    if r["diagnostic_bundled_cost_per_unit"]
                    else float("nan")
                ),
                ...
                "ch4_physical_kg_per_mwh": r.get(
                    "diagnostic_ch4_physical_kg_per_mwh", float("nan")
                ),
                "n2o_physical_kg_per_mwh": r.get(
                    "diagnostic_n2o_physical_kg_per_mwh", float("nan")
                ),
...
def _emit_energy_intensity_csv(rows: list[dict], granular_dir: Path) -> Path:
    ...
    flat_rows = []
    for r in rows:
        archetype = _METHOD_ID_TO_ARCH.get(
            r["method_id"], r["method_id"].split("__")[-1]
        )
...
def main(run: Path, out: Path | None = None, run_id_prefix: str | None = None) -> None:
    ...
    :param run_id_prefix: Filter run dirs to those starting with this prefix, when
        ``run`` holds more than one production run.
    """
    logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")
    layout = OutputLayout(run)
    workbook_cache = Env.from_env().workbook_cache
    out = out or layout.exports / "sharp"
    out.mkdir(parents=True, exist_ok=True)

    runs = _find_archetype_runs(layout.runs, run_id_prefix=run_id_prefix)
    ...
    methods_path = _emit_methods_csv(out)
    method_years_path = _emit_method_years_csv(all_rows, out)
    diagnostics_path = _emit_diagnostics_csv(all_rows, out)
    provenance_path = _emit_provenance_csv(out)
    granular_dir = out.parent / "granular"
