...
def _load_fuel_price_tables(workbook_cache: Path) -> dict[str, pd.DataFrame]:
    ...
    The v6.0 to v7.4 schema normalisation renamed the coal/gas/biomass price
    tables. This tries the v7.4-canonical names first and falls back to the
    older v6.0 names, so the post-processor works against either cache
    vintage:
...
def _check_carbon_pricing_columns(gens: pd.DataFrame, pypsa_friendly_dir: Path) -> None:
    ...
    metadata columns the translator adds. Without them the postprocessor
    degrades to capture_rate=0 (no CCS credit) silently; surface the
    ...
    if missing:
        log.warning(
            ...
            "will degrade to zero. Re-translate with a build that emits these "
            "columns to pick them up.",
            pypsa_friendly_dir,
            sorted(missing),
        )
...
def _assemble_row(
    ...
    *,
    ...
    `output_cost_per_unit` is the fuel-only-stripped intensity, kept for
    backward compatibility with downstream readers.
    `cost_per_unit_excl_fuel_and_carbon` is the carbon-stripped intensity
    (capex + non-fuel non-carbon opex) / annual_mwh - T&S is deliberately
    retained. If carbon_price == 0 the two are numerically equal.
    """
