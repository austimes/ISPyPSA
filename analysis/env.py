...
workstation (SMB share) and on the cluster (NFS), so nothing is copied between hosts::
    ...
    $IO_DIR/inputs/                      IASR workbook, parsed cache, trace stores, trajectory trace dirs
    $IO_DIR/runs/<run_set>/<stamp>/      one directory per launch, holding every product of that launch
...
class OutputLayout:
    ...
    :param root: The launch directory, ``$IO_DIR/runs/<run_set>/<stamp>``.
    """
...
class Env:
    ...
    def inputs(self) -> Path:
        """Shared input stores, common to every run set."""
        return self.io_dir / "inputs"
    ...
    @property
    def runs(self) -> Path:
        """Root of every run set."""
        return self.io_dir / "runs"

    def run_set(self, name: str) -> Path:
        """Directory holding every launch of one run set."""
        return self.runs / name

    def new_run(self, run_set: str) -> OutputLayout:
        """Create and return a freshly stamped launch directory under ``run_set``."""
        layout = OutputLayout(
            self.run_set(run_set) / datetime.now().strftime(RUN_STAMP_FORMAT)
        )
