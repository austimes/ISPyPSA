...
* VRE traces (``project/``, ``zone/``) are symlinked to one shared source store, so wind and solar are bit-identical
  between trajectories. Use ``--vre-mode copy`` to retain the same bytes on systems without symlink privileges.
...
Usage:
    uv run python analysis/benchmarks/build_trajectory_demand_dirs.py \\
        --source data/trace_data_final/isp_2026 \\
        --out-root /scratch/extension_campaign_traces \\
        --plan path/to/demand-plan.json
"""
...
def _reset_dataset_dir(base_dir: Path, dataset_dir: str, out_root: Path) -> Path:
    ...
    if base_dir.is_symlink() or base_dir.resolve().parent != out_root.resolve():
        raise ValueError(
            f"Trace output must be an immediate child of the output root: {base_dir}"
        )
    if base_dir.exists():
...
def _build_demand_dir(
    ...
    dataset_dir = _reset_dataset_dir(
        stores.out_root / f"{trajectory}_{milestone_year}",
        stores.dataset_dir,
        stores.out_root,
    )
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="symlink",
        help="Link the shared VRE store, or copy it explicitly when directory symlinks are unavailable",
    )
    parser.add_argument(
        "--reference-year",
        type=int,
...
def main(argv: list[str] | None = None) -> None:
    ...
    source_root, output_root = args.source.resolve(), args.out_root.resolve()
