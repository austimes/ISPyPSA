...
facilities only. This patch restores the rest of the workbook's PHES menu:
  ...
  6. Kidston and Phoenix Pumped Hydro appended to `ecaa_batteries` as fixed
     committed/policy units (same pathway as the four `pumped_storage_fix`
...
extend the menu to one and two weeks of storage, and are offered on every
solve so the cost ladder stays comparable across runs.
