"""Correct the new-entrant biomass feedstock cost to a scale-appropriate,
sourced delivered cost.
...
Applied as one of the fork-specific model patches (see
``analysis/model/__init__.py``), alongside the biomass capacity cap, so
every run sees consistent biomass availability *and* cost economics.
"""
