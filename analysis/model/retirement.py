...
capacity variable on the existing fleet:
    ...
    the capacity each period -- passing 0 gives a strict mechanism with no
    economics; passing the AEMO fixed-OPEX means shedding an idle unit pays,
    since coal units run idle at $550/MWh and the keeping cost, not operating
    savings, is what drives retirement.
...
def make_existing_reducible(
    ...
    generators are also non-extendable (~100 GW each), and applying a
    keeping-cost FOM to those would let the LP "retire" load-shedding
    capacity. Targeting by ECAA
    ...
    # Keeping cost: a per-unit FOM dict (each unit's own $/MW/yr) or a scalar
    # (0 gives a strict mechanism with no economics). As capital_cost it is
    # multiplied by the investment-period objective weighting (so it is a
    # per-period recurring cost) -- see the span-weighting override in
    # instrumented_runner.
...
def save_retention_floor(
    ...
    df = pd.DataFrame({"name": list(floor.keys()), "retained_mw": list(floor.values())})
    df.to_parquet(out_dir / _FLOOR_FILENAME, index=False)


def load_retention_floor(retention_root: Path, before_year: int) -> dict[str, float]:
