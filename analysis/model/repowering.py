...
original closure year: without repowering, a cohort of mid-2040s-closing
wind farms drops out faster than new entrants build to replace them,
...
performance modelling; only the cost side is implemented here):
  ...
  3. **Single repowering cycle.** Real repowering can repeat (2025 to
     2045 to 2065 turbines); this implementation limits each asset to
     one cycle.
...
def apply(ispypsa_tables: dict[str, pd.DataFrame], config) -> dict[str, pd.DataFrame]:
    ...
    ecaa.loc[vre_mask, "closure_year"] = (
        ecaa.loc[vre_mask, "closure_year"] + _LIFE_EXTENSION_YEARS
    )
...
def _log_summary(ecaa: pd.DataFrame, vre_mask: pd.Series) -> None:
    ...
    log.info(
        f"repowering: extended closure_year by {_LIFE_EXTENSION_YEARS}y and added annualised premium for {len(affected)} VRE rows: {sorted(by_fuel.items())}"
    )
