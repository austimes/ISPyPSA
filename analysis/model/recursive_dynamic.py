...
Solving each milestone year as an independent static greenfield problem
ignores capacity built in earlier years. Locking 2040 capacity into a 2045
solve instead shifts emissions ~+17% and renewable share ~-7.7 pp, so a
greenfield chain does not give a realistic emissions and capacity
trajectory. This module implements the write-back: each year's newly-built
tranche becomes part of the next year's *existing* fleet, accumulating
across the chain.
...
def inject_carried_tranches(
    ...
    write-back. This function mutates the in-memory DataFrames in place;
    the caller is responsible for any CSV rewrite for inspectability.
    """
    diag = {
