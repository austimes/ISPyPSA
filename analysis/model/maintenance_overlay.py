...
def _log_summary(ecaa: pd.DataFrame, first_period: int) -> None:
    ...
    if affected.empty:
        log.info(
            f"maintenance_overlay: no ECAA thermal units within EOL window of first period {first_period}"
        )
        return
    by_fuel = affected.groupby("fuel_type").size().to_dict()
    log.info(
        f"maintenance_overlay: ageing premium applied to {len(affected)} ECAA rows by fuel_type: {sorted(by_fuel.items())}"
    )
