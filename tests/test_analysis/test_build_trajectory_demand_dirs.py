"""Tests for the extension campaign's per-trajectory demand trace directory builder."""
...
from analysis.benchmarks.build_trajectory_demand_dirs import _reset_dataset_dir, main
...
def test_unit_scalar_still_rewrites_the_demand_file(built, source_store):
    written = (
        built / "unit_flat_2049" / DATASET_DIR / DEMAND_PARTITION / "data_0.parquet"
    )
...
def test_extension_year_appends_fy2050_shifted_ten_years(built, source_store):
    written = (
        built / "unit_scaled_2060" / DATASET_DIR / DEMAND_PARTITION / "data_0.parquet"
    )
...
def test_vre_matches_one_shared_store(built, source_store, vre_mode):
    ...
    extension_links = [
        built / "unit_flat_2060" / DATASET_DIR / subdir
        for subdir in ("project", "zone")
    ]
...
def test_tracedirs_file_lists_one_token_per_milestone_in_year_order(built):
    tokens = (
        (built / "tracedirs" / "unit_scaled.txt").read_text(encoding="utf-8").split()
    )

    expected = [
        f"{year}:{(built / f'unit_scaled_{year}').resolve().as_posix()}"
