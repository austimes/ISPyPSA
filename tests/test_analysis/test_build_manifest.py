...
from analysis.extension_campaign.build_manifest import (
    build_caps_table,
    build_chain_table,
    write_manifest,
)
...
def test_chains_tsv_orders_price_chains_before_cap_chains(plan, tmp_path):
    ...
    write_manifest(caps, chains, tmp_path)
    ...
    assert len(lines) == 20
    assert lines[0] == "ext_central_c0\tiasr_central\t--carbon-price 0"
    assert lines[1] == "ext_stress_c0\tiasr_stress\t--carbon-price 0"
    assert lines[2] == "ext_central_c150\tiasr_central\t--carbon-price 150"
    assert lines[7] == "ext_stress_c550\tiasr_stress\t--carbon-price 550"
...
def test_chain_args_carry_every_milestone_tonnage(plan, csv_str_to_df):
    ...
    expected = csv_str_to_df("""
        row,  run_id,               trajectory,    chain,    stage,  args
        16,   ext_central_cap0001,  iasr_central,  cap0001,  2b,     --co2-cap-t-schedule 2030:19983600 2040:2671572 2050:332150 2060:392210
    """)
