...
def test_translate_sink_tranches_keeps_an_all_zero_variant(tmp_path):
    ...
    csv = _write(
        tmp_path, "tranches.csv", _TRANCHE_CSV + "carbonnet,2050,0.0,18.5\n"
    )
...
def test_translate_sink_tranches_raises_on_missing_columns(tmp_path):
    ...
    with pytest.raises(ValueError, match=r"missing columns: \['cap_kt', 'storage_\$/t'\]"):
...
def test_translate_transport_adders_raises_on_duplicate_sub_region(tmp_path):
    ...
    with pytest.raises(ValueError, match=r"more than one sink to sub-regions: \['SNW'\]"):
